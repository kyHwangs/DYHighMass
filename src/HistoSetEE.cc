...
void HistoSetEE::Fill2DHisto(std::string name, double value1, double value2, double weight) {

  if (fHistSet2D.find(name) == fHistSet2D.end()) {
    std::cout << "Error: HistoSetEE::Fill2DHisto: Unknown histogram name: " << name << std::endl;
    exit(1);
  }
